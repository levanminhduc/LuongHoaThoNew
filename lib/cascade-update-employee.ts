...
export async function cascadeUpdateEmployeeId(
  ...
  try {
    ...
    const rollbackData: Array<{table: string, field: string, oldValue: string, newValue: string}> = []
    ...
    try {
      ...
      if (employeesError) throw new Error(`Employees update failed: ${employeesError.message}`)
      updatedStats.employees = 1
      rollbackData.push({table: "employees", field: "employee_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
      ...
      if (stats.payrolls > 0) {
        ...
        if (payrollsError) throw new Error(`Payrolls update failed: ${payrollsError.message}`)
        updatedStats.payrolls = stats.payrolls
        rollbackData.push({table: "payrolls", field: "employee_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
      ...
      if (stats.signature_logs > 0) {
        ...
        if (signatureError) throw new Error(`Signature logs update failed: ${signatureError.message}`)
        updatedStats.signature_logs = stats.signature_logs
        rollbackData.push({table: "signature_logs", field: "employee_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.signature_logs} records in signature_logs`)
      }
      ...
      if (stats.department_permissions_employee > 0) {
        ...
        if (deptPermError) throw new Error(`Department permissions (employee_id) update failed: ${deptPermError.message}`)
        updatedStats.department_permissions_employee = stats.department_permissions_employee
        rollbackData.push({table: "department_permissions", field: "employee_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.department_permissions_employee} records in department_permissions (employee_id)`)
      }
      ...
      if (stats.management_signatures > 0) {
        ...
        if (mgmtSigError) throw new Error(`Management signatures update failed: ${mgmtSigError.message}`)
        updatedStats.management_signatures = stats.management_signatures
        rollbackData.push({table: "management_signatures", field: "signed_by_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.management_signatures} records in management_signatures`)
      }
      ...
      if (stats.department_permissions_granted_by > 0) {
        ...
        if (deptPermGrantedError) throw new Error(`Department permissions (granted_by) update failed: ${deptPermGrantedError.message}`)
        updatedStats.department_permissions_granted_by = stats.department_permissions_granted_by
        rollbackData.push({table: "department_permissions", field: "granted_by", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.department_permissions_granted_by} records in department_permissions (granted_by)`)
      }
      ...
      if (stats.access_logs_employee_accessed > 0) {
        ...
        if (accessLogsEmpError) throw new Error(`Access logs (employee_accessed) update failed: ${accessLogsEmpError.message}`)
        updatedStats.access_logs_employee_accessed = stats.access_logs_employee_accessed
        rollbackData.push({table: "access_logs", field: "employee_accessed", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.access_logs_employee_accessed} records in access_logs (employee_accessed)`)
      }
      ...
      if (stats.access_logs_user_id > 0) {
        ...
        if (accessLogsUserError) throw new Error(`Access logs (user_id) update failed: ${accessLogsUserError.message}`)
        updatedStats.access_logs_user_id = stats.access_logs_user_id
        rollbackData.push({table: "access_logs", field: "user_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.access_logs_user_id} records in access_logs (user_id)`)
      }
      ...
      if (stats.payroll_audit_logs > 0) {
        ...
        if (auditLogsError) throw new Error(`Payroll audit logs update failed: ${auditLogsError.message}`)
        updatedStats.payroll_audit_logs = stats.payroll_audit_logs
        rollbackData.push({table: "payroll_audit_logs", field: "employee_id", oldValue: oldEmployeeId, newValue: newEmployeeId})
        console.log(`Updated ${stats.payroll_audit_logs} records in payroll_audit_logs`)
      }
      ...
      for (let i = rollbackData.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          console.error(`Rollback failed for ${rollback.table}.${rollback.field}:`, rollbackError)
        }
      }
    ...
    const totalAffected = Object.values(updatedStats).reduce((sum, count) => sum + count, 0)
