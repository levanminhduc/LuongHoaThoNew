...
export async function calculateEmployeeCompletion(month: string): Promise<EmployeeCompletion> {
  ...
  if (payrollError) {
    throw new Error(`Error fetching employees with payroll: ${payrollError.message}`)
  }
...
export async function checkSignatureEligibility(
  ...
  try {
    ...
    try {
      ...
      console.log("Management signatures table not available - proceeding with eligibility check")
    }
...
export async function getManagementSignatureStatus(month: string): Promise<MonthStatus> {
  try {
    const employeeCompletion = await calculateEmployeeCompletion(month)
    ...
    try {
      ...
      console.log("Management signatures table not available - using null values")
    }
    ...
    const remainingSignatures = ['giam_doc', 'ke_toan', 'nguoi_lap_bieu'].filter(
      type => !managementSignatures[type as keyof typeof managementSignatures]
    )
    ...
    return {
      ...
      summary: {
        ...
        employee_completion_required: employeeCompletion.is_100_percent_complete
      }
    }
  } catch (error) {
    throw new Error(`Error getting signature status: ${error instanceof Error ? error.message : String(error)}`)
  }
}

export function validateSignatureType(signatureType: string): signatureType is 'giam_doc' | 'ke_toan' | 'nguoi_lap_bieu' {
...
export function canUserSignType(userRole: string, signatureType: string): boolean {
