...
export interface UseHeaderMappingReturn {
  ...
  mapHeaders: (fields: string[], configId?: number, options?: HeaderMappingOptions) => HeaderMappingResult
  getReverseMapping: (configId?: number, includeDefaults?: boolean) => Record<string, string>
  formatFieldName: (fieldName: string) => string
  
  // Validation
  validateMapping: (configId: number, requiredFields?: string[]) => {
    isValid: boolean
    errors: string[]
    warnings: string[]
    coverage: number
  ...
  generatePreview: (fields: string[], configId?: number) => {
    preview: Array<{
      field: string
      header: string
...
export const useHeaderMapping = (): UseHeaderMappingReturn => {
  ...
  const mergeConfigurations = useCallback((
    ...
    const configs = configIds
      ...
      .filter((config): config is MappingConfiguration => config !== undefined)
...
export const useConfigHeaderMapping = (configId?: number) => {
  ...
  const getHeader = useCallback((field: string): string => {
    return headers[field] || formatFieldNameAsHeader(field)
  }, [headers])

  const getField = useCallback((header: string): string | undefined => {
...
export const useFieldValidation = (requiredFields: string[] = []) => {
  ...
  const validateFields = useCallback((fields: string[], configId?: number) => {
...
export const useHeaderMappingPreview = (
  ...
  const getFieldPreview = useCallback((field: string) => {
    return preview.preview.find(p => p.field === field)
  }, [preview])

  const getHeadersBySource = useCallback((source: 'configuration' | 'default' | 'fallback') => {
    return preview.preview.filter(p => p.source === source)
  }, [preview])

  const getLowConfidenceFields = useCallback((threshold = 50) => {
...
export const useHeaderMappingStats = () => {
  ...
  const stats = useMemo(() => {
    ...
    const avgMappingsPerConfig = totalConfigs > 0 ? totalMappings / totalConfigs : 0
