...
export const useMappingConfig = (): UseMappingConfigReturn => {
  ...
  const configById = useCallback((id: number) => {
    return store.configurations.find(config => config.id === id)
  }, [store.configurations])

  const configByName = useCallback((name: string) => {
...
export const useAutoLoadConfigurations = (options?: {
  ...
  const { loadConfigurations, loadDefaultConfiguration, isLoading, error } = useMappingConfig()
...
export const useCurrentMappingConfig = () => {
  const { currentConfig, defaultConfig, applyConfiguration, clearCurrentConfig } = useMappingConfig()
...
export const useConfigNotifications = () => {
  ...
  const getNotificationsByType = useCallback((type: ConfigNotification['type']) => {
...
export const useConfigValidation = () => {
  ...
  const validateConfiguration = useCallback((config: Partial<MappingConfiguration>) => {
    const errors: string[] = []
    ...
    const duplicateName = configurations.find(c => 
      c.config_name === config.config_name && c.id !== config.id
    )
    if (duplicateName) {
    ...
    if (!config.field_mappings || config.field_mappings.length === 0) {
      ...
      if (duplicateFields.length > 0) {
        errors.push(`Có field mappings trùng lặp: ${duplicateFields.join(', ')}`)
      }
      ...
      if (duplicateColumns.length > 0) {
        errors.push(`Có Excel columns trùng lặp: ${duplicateColumns.join(', ')}`)
      }
    }
  ...
  const previewConfiguration = useCallback((config: MappingConfiguration) => {
    ...
    const highConfidenceMappings = fieldMappings.filter(m => m.confidence_score >= 80)
    const lowConfidenceMappings = fieldMappings.filter(m => m.confidence_score < 50)
    ...
    return {
      ...
      averageConfidence: fieldMappings.length > 0 
        ? Math.round(fieldMappings.reduce((sum, m) => sum + m.confidence_score, 0) / fieldMappings.length)
        : 0,
      mappingTypes: {
...
export const useConfigSearch = () => {
  ...
  const searchConfigurations = useCallback((query: string) => {
  ...
  const filterConfigurations = useCallback((filters: {
    isActive?: boolean
    isDefault?: boolean
    createdBy?: string
    ...
    return configurations.filter(config => {
      if (filters.isActive !== undefined && config.is_active !== filters.isActive) {
        return false
      }
      if (filters.isDefault !== undefined && config.is_default !== filters.isDefault) {
        return false
      }
...
export const useConfigStats = () => {
  ...
  const stats = useMemo(() => {
    ...
    const withFieldMappings = configurations.filter(c => (c.field_mappings?.length || 0) > 0).length
    
    const creators = [...new Set(configurations.map(c => c.created_by))]
    const avgFieldMappings = total > 0 
      ? Math.round(configurations.reduce((sum, c) => sum + (c.field_mappings?.length || 0), 0) / total)
      : 0
...
export const useConfigExportImport = () => {
  ...
  const exportConfiguration = useCallback((configId: number) => {
  ...
  const importConfiguration = useCallback(async (file: File) => {
    try {
      ...
      throw new Error(`Lỗi import configuration: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }, [saveConfiguration])
