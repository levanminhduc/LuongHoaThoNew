...
export const SignatureTypeSchema = z.enum(['giam_doc', 'ke_toan', 'nguoi_lap_bieu'], {
...
export type ManagementSignatureRequest = z.infer<typeof ManagementSignatureRequestSchema>
...
export type SignatureProgressParams = z.infer<typeof SignatureProgressParamsSchema>
export type SignatureType = z.infer<typeof SignatureTypeSchema>

export function validateManagementSignatureRequest(data: unknown): ManagementSignatureRequest {
  try {
...
export function validateSignatureHistoryQuery(query: Record<string, string | undefined>): SignatureHistoryQuery {
  try {
...
export function validateSignatureStatusParams(params: unknown): SignatureStatusParams {
  try {
...
export function validateSignatureProgressParams(params: unknown): SignatureProgressParams {
  try {
...
export const ALLOWED_SIGNATURE_ROLES = ['admin', 'giam_doc', 'ke_toan', 'nguoi_lap_bieu'] as const

export function isAllowedSignatureRole(role: string): role is typeof ALLOWED_SIGNATURE_ROLES[number] {
  return ALLOWED_SIGNATURE_ROLES.includes(role as any)
}

export function canRoleSignType(role: string, signatureType: SignatureType): boolean {
