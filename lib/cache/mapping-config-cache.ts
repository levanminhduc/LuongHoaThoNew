...
export const cacheUtils = {
  ...
  cacheConfiguration: (config: MappingConfiguration) => {
    ...
    mappingConfigCache.set(CacheKeys.CONFIG_BY_NAME(config.config_name), config)
    
    if (config.field_mappings) {
      mappingConfigCache.set(CacheKeys.FIELD_MAPPINGS(config.id!), config.field_mappings)
    }
  },

  // Optimistic configuration update
  optimisticConfigUpdate: (id: number, updates: Partial<MappingConfiguration>) => {
    const existing = mappingConfigCache.get<MappingConfiguration>(CacheKeys.CONFIG_BY_ID(id))
    if (existing) {
  ...
  getCachedConfiguration: (id: number): MappingConfiguration | null => {
    return mappingConfigCache.get<MappingConfiguration>(CacheKeys.CONFIG_BY_ID(id))
  },

  // Get cached configuration by name
  getCachedConfigurationByName: (name: string): MappingConfiguration | null => {
    return mappingConfigCache.get<MappingConfiguration>(CacheKeys.CONFIG_BY_NAME(name))
  },

  // Get cached configuration list
  getCachedConfigurationList: (): MappingConfiguration[] | null => {
    return mappingConfigCache.get<MappingConfiguration[]>(CacheKeys.CONFIGURATIONS)
  },

  // Get cached default configuration
  getCachedDefaultConfiguration: (): MappingConfiguration | null => {
    return mappingConfigCache.get<MappingConfiguration>(CacheKeys.DEFAULT_CONFIG)
  },
