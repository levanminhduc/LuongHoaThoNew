...
import { isAllowedSignatureRole, canRoleSignType, type SignatureType } from "@/lib/signature-validation"
...
export function verifyManagementSignatureAuth(request: NextRequest): ManagementSignatureAuth | null {
  ...
  return {
    ...
    canAccessSignatureHistory: () => {
      return ['admin', 'giam_doc', 'ke_toan', 'nguoi_lap_bieu'].includes(auth.user.role)
    },
...
export function getRateLimitInfo(endpoint: string, userRole: string): RateLimitInfo {
