...
export interface PayrollSignatureLog {
  employee_id: string;
  salary_month: string;
...
export interface PayrollManagementSignature {
  signed_by_name?: string;
  signed_at?: string;
}

export interface PayrollExportContext {
  payrollData: PayrollExportRecord[];
  signatureLogsMap: Map<string, PayrollSignatureLog>;
...
export function buildPayrollExportSheet(context: PayrollExportContext): {
  ...
  const dataRows = payrollData.map(
    (record: PayrollExportRecord, index: number) => {
