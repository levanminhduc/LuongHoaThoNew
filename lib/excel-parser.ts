  ...
  try {
    ...
    rows.forEach((row, index) => {
      ...
      const data: PayrollData = {
        ...
        cccd: cccd,
        position:
          columnIndexes.position !== undefined
            ? String(row[columnIndexes.position] || "")
            : "",
        salary_month:
          columnIndexes.salary_month !== undefined
            ? String(row[columnIndexes.salary_month] || "")
            : "",
        total_income:
          columnIndexes.total_income !== undefined
            ? Number.parseFloat(
                String(row[columnIndexes.total_income] || "0"),
              ) || 0
            : 0,
        deductions:
          columnIndexes.deductions !== undefined
            ? Number.parseFloat(String(row[columnIndexes.deductions] || "0")) ||
              0
            : 0,
        net_salary:
          columnIndexes.net_salary !== undefined
            ? Number.parseFloat(String(row[columnIndexes.net_salary] || "0")) ||
              0
            : 0,
