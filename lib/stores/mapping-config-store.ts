  ...
  devtools(
    immer((set, get) => ({
      ...
      loadConfigurations: async () => {
        ...
        if (
          cachedConfigs &&
          isCacheValid(state.cacheTimestamp, state.cacheExpiry)
        ) {
          set((state) => {
            state.configurations = cachedConfigs;
            state.defaultConfig =
              cachedConfigs.find((c) => c.is_default) || null;
        ...
        try {
          ...
          set((state) => {
            ...
            state.defaultConfig =
              configurations.find((c) => c.is_default) || null;
            state.isLoading = false;
      ...
      loadDefaultConfiguration: async () => {
        ...
        try {
          ...
          const defaultConfig =
            get().configurations.find((c) => c.is_default) || null;
      ...
      saveConfiguration: async (config) => {
        ...
        try {
          ...
          mappingConfigCache.clearOptimistic(
            CacheKeys.CONFIG_BY_ID(Date.now()),
          );
        ...
        try {
          ...
          get().addNotification({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Lỗi khi cập nhật configuration",
            read: false,
          });
