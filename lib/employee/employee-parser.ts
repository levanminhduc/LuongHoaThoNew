...
export function parseEmployeeExcelFile(buffer: Buffer): ImportResult {
  try {
    ...
    rows.forEach((row, index) => {
      ...
      if (seenEmployeeIds.has(employeeId)) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error: "Mã nhân viên bị trùng trong file",
        });
        return;
      }
      ...
      if (employeeId.length > 50) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error: "Mã nhân viên quá dài (tối đa 50 ký tự)",
        });
        return;
      }
      ...
      if (fullName.length > 255) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error: "Họ tên quá dài (tối đa 255 ký tự)",
        });
        return;
      }
      ...
      if (cccd.length > 20) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error: "Số CCCD quá dài (tối đa 20 ký tự)",
        });
        return;
      }
      ...
      if (department.length > 100) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error: "Tên phòng ban quá dài (tối đa 100 ký tự)",
        });
        return;
      }
      ...
      const phoneNumber =
        columnIndexes.phone_number !== undefined
          ? String(row[columnIndexes.phone_number] || "").trim()
          : "";
      ...
      if (
        phoneNumber &&
        (phoneNumber.length > 15 || !/^[0-9+\-\s()]*$/.test(phoneNumber))
      ) {
        errors.push({
          row: rowNumber,
          employee_id: employeeId,
          error:
            "Số điện thoại không hợp lệ (chỉ chấp nhận số, +, -, khoảng trắng, dấu ngoặc)",
        });
        return;
      }
    ...
    throw new Error(
      `Lỗi khi đọc file Excel: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
