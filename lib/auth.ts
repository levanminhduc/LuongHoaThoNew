...
export async function verifyAdminCredentials(username: string, password: string): Promise<AdminUser | null> {
  ...
  if (username.toLowerCase() === 'admin') {
    console.error(`🚫 SECURITY BLOCK: Admin credentials check blocked for username 'admin'`)
    return null
  }
...
export async function authenticateUser(username: string, password: string): Promise<AuthResult> {
  try {
    ...
    if (username.toLowerCase() === 'admin') {
      console.error(`🚫 SECURITY BLOCK: Login attempt with blocked username 'admin'`)
      return {
        success: false,
...
function getPermissionsByRole(role: string): string[] {
  switch (role) {
    ...
    case 'van_phong':
      return ['VIEW_EMPLOYEES', 'MANAGE_EMPLOYEES', 'VIEW_REPORTS', 'EXPORT_DATA']
    default:
      return []
  }
}
 ...
 */
export async function createAdminUser(username: string, password: string, role: string = 'admin'): Promise<boolean> {
  try {
    const supabase = createServiceClient()
