...
export function formatHighPrecisionNumber(
  ...
  const maxDecimals = options?.maxDecimalPlaces ?? config?.maxDecimalPlaces ?? 10
 ...
 */
export function formatTimeValue(value: number | string | null | undefined): string {
...
export function parseHighPrecisionNumber(
  ...
  const config = fieldName ? FIELD_PRECISION_CONFIG[fieldName] : FIELD_PRECISION_CONFIG['default_money']
  ...
  try {
    ...
    const rounded = Math.round(parsed * Math.pow(10, config.maxDecimalPlaces)) / Math.pow(10, config.maxDecimalPlaces)
    ...
    if (config.fieldType === 'coefficient') {
      ...
      if (fieldName?.includes('gio') && rounded > 744) {
        warnings.push('Hours value seems high for a month (744 = 31 days × 24 hours)')
      }
    }
...
export function validateDecimal15_10(value: number): {
  ...
  if (Math.abs(value) >= 100000) {
    return {
      isValid: false,
      error: 'Value exceeds DECIMAL(15,10) integer limit (max 5 digits before decimal)'
    }
  }
  ...
  if (decimalPart.length > 10) {
    return {
      isValid: false,
      error: 'Value exceeds DECIMAL(15,10) precision limit (max 10 decimal places)'
    }
  }
