...
export function mapFieldsToHeaders(
  ...
  const averageConfidence = includeConfidence && mappedCount > 0 
    ? totalConfidence / mappedCount 
    : 0
...
export function validateHeaderMapping(
  ...
  if (!configuration.field_mappings || configuration.field_mappings.length === 0) {
  ...
  const duplicateDbFields = dbFields.filter((field, index) => dbFields.indexOf(field) !== index)
  ...
  const excelHeaders = configuration.field_mappings.map(m => m.excel_column_name.toLowerCase())
  const duplicateHeaders = excelHeaders.filter((header, index) => excelHeaders.indexOf(header) !== index)
  ...
  const lowConfidenceFields = configuration.field_mappings.filter(m => m.confidence_score < 50)
  if (lowConfidenceFields.length > 0) {
    warnings.push(`${lowConfidenceFields.length} fields có confidence thấp (<50%)`)
  }
...
export function mergeHeaderMappings(
  ...
  conflictResolution: 'highest_confidence' | 'latest' | 'first' = 'highest_confidence'
): Record<string, string> {
  const mergedHeaders: Record<string, string> = {}
  const fieldMappings = new Map<string, { header: string; confidence: number; timestamp: number }>()
  ...
  configurations.forEach((config, configIndex) => {
    ...
    config.field_mappings.forEach(mapping => {
      ...
      const timestamp = config.updated_at ? new Date(config.updated_at).getTime() : configIndex
...
export function generateHeaderMappingPreview(
  ...
  fields.forEach(field => {
    ...
    const configMapping = configuration?.field_mappings?.find(m => m.database_field === field)
    if (configMapping) {
