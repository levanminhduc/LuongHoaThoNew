...
export interface SyncEvent {
  type: 'config_created' | 'config_updated' | 'config_deleted' | 'default_changed' | 'cache_invalidated'
  configId?: number
  data?: any
  timestamp: number
...
class MappingConfigSyncManager {
  ...
  subscribe(callback: (event: SyncEvent) => void, filter?: (event: SyncEvent) => boolean): string {
  ...
  broadcast(event: Omit<SyncEvent, 'timestamp' | 'source'>): void {
    ...
    try {
      localStorage.setItem('mapping-config-sync-event', JSON.stringify({ ...syncEvent, source: 'external' }))
  ...
  private async checkForExternalUpdates(): Promise<void> {
    try {
      ...
      const response = await fetch('/api/admin/mapping-configurations?timestamp_only=true', {
        headers: {
  ...
  triggerConfigUpdated(configId: number, updates: Partial<MappingConfiguration>): void {
    this.broadcast({
      type: 'config_updated',
      configId,
      data: updates
...
export const useMappingConfigSync = () => {
  const [isConnected, setIsConnected] = React.useState(syncManager.isConnected())
  const [lastSync, setLastSync] = React.useState(syncManager.getLastSyncTimestamp())
  const [subscriptionCount, setSubscriptionCount] = React.useState(syncManager.getSubscriptionCount())
  ...
  return {
    ...
    triggerCacheInvalidated: syncManager.triggerCacheInvalidated.bind(syncManager),
  }
}
...
export const syncUtils = {
  ...
  subscribeToConfig: (configId: number, callback: (event: SyncEvent) => void) => {
    return syncManager.subscribe(callback, (event) => event.configId === configId)
  },

  // Subscribe to default config changes
  subscribeToDefaultChanges: (callback: (event: SyncEvent) => void) => {
    return syncManager.subscribe(callback, (event) => event.type === 'default_changed')
  },

  // Subscribe to all config changes
  subscribeToAllChanges: (callback: (event: SyncEvent) => void) => {
    return syncManager.subscribe(callback, (event) => 
      ['config_created', 'config_updated', 'config_deleted'].includes(event.type)
    )
  },
