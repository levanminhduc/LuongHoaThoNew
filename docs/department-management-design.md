...
async function getAccessibleDepartments(user: User): Promise<string[]> {
  ...
  const { data: managed } = await db.from('departments').select('code').eq('manager_id', user.employee_id);
  rootDepts.push(...managed.map(d => d.code));
  
  // Case B: Quyền bổ sung từ bảng department_permissions (quyền kiêm nhiệm)
  const { data: granted } = await db.from('department_permissions').select('department_code').eq('employee_id', user.employee_id);
  ...
  const { data: allDepts } = await db.rpc('get_recursive_downstream_departments', { root_codes: rootDepts });
