...
> | **Q4** Testability | Partial (22 file) | **Partial** | **62 test file / 1.061 test case** (audit gốc 21 file / 94 test). Mọi truy vấn DB giờ test được qua parity test; orchestration của `admin/employees/[id]` đã rút thành service và test được, phần còn lại trong route handler vẫn chưa |
...
> | **A2** Controller mỏng | Fail (TB 236 dòng) | **Partial** (TB **205**) | 15.191 dòng / 74 route. Route không còn chuỗi select hay tên cột DB, nhưng chưa đạt ngưỡng 100 dòng của CLAUDE.md — phần còn lại là builder XLSX dựng workbook ngay trong route, thuộc nhóm 16 chưa làm |
