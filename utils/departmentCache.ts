...
class DepartmentCache {
   ...
   */
  private static generateCacheKey(departmentName: string, month: string): string {
  ...
  static setCacheData(
    ...
    try {
      ...
      console.log(`[DepartmentCache] Data cached for ${departmentName} - ${month}`, {
        cacheKey,
  ...
  static getCacheData(departmentName: string, month: string): any | null {
    try {
      ...
      if (!cachedItem) {
        console.log(`[DepartmentCache] No cache found for ${departmentName} - ${month}`)
        return null
      }
      ...
      if (now > cacheData.expiresAt) {
        console.log(`[DepartmentCache] Cache expired for ${departmentName} - ${month}`, {
      ...
      console.log(`[DepartmentCache] Cache hit for ${departmentName} - ${month}`, {
        cachedAt: new Date(cacheData.timestamp).toLocaleString('vi-VN'),
        expiresAt: new Date(cacheData.expiresAt).toLocaleString('vi-VN'),
        remainingTime: Math.round((cacheData.expiresAt - now) / (1000 * 60)) + ' minutes'
      })
  ...
  static clearCache(departmentName: string, month: string): void {
    try {
      ...
      console.log(`[DepartmentCache] Cache cleared for ${departmentName} - ${month}`)
    } catch (error) {
  ...
  static clearAllCache(): void {
    try {
      ...
      const departmentCacheKeys = keys.filter(key => key.startsWith(this.CACHE_PREFIX))
      
      departmentCacheKeys.forEach(key => {
        localStorage.removeItem(key)
      })
      
      console.log(`[DepartmentCache] Cleared ${departmentCacheKeys.length} cache entries`)
    } catch (error) {
  ...
  static forceRefresh(departmentName: string, month: string): null {
    ...
    console.log(`[DepartmentCache] Force refresh triggered for ${departmentName} - ${month}`)
    return null
  }
}
