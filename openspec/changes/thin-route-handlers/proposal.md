...
- **187 lệnh `.from()` nằm thẳng trong 55 route handler**, chạm 17 bảng. Hai bảng chiếm quá nửa: `employees` 62 lệnh, `payrolls` 49.
...
Ba repository đã có (`payroll`, `bonus`, `employee`) chứng minh khuôn làm việc được và đã có test — vấn đề là mới phủ 15/187 lệnh.
...
- Mở rộng `employee-repository` và `payroll-repository` (2 nhóm lớn nhất, 111/187 lệnh).
...
- `app/api/**`: 55 route bị sửa, ước tính giảm ~4.000 dòng.
