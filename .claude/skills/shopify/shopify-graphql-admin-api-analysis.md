...
mutation {
  productCreate(
    input: { title: "New Product", productType: "Apparel", vendor: "Acme Corp" }
  ) {
    product {
      id
      title
...
```graphql
query {
  products(first: 20, query: "product_type:Apparel AND tag:summer") {
    edges {
      node {
        id
        title
...
query {
  products(first: 10) {
    # Cost: ~11 points (1 for query + 10 for items)
    edges {
      node {
        id
        title
      ...
      lineItemsByFulfillmentOrder: [
        {
          ...
          fulfillmentOrderLineItems: [
            { id: "gid://shopify/FulfillmentOrderLineItem/456", quantity: 2 }
          ]
        }
      ]
      trackingInfo: {
