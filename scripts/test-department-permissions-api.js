...
async function testGetDepartments() {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/admin/departments?include_stats=true`, {
      headers: {
    ...
    if (response.ok) {
      ...
      if (data.departments && data.departments.length > 0) {
        ...
        data.departments.forEach(dept => {
          console.log(`  - ${dept.name} (${dept.employeeCount} employees, ${dept.payrollCount} payrolls)`)
        })
      }
      return data.departments
    } else {
      console.error('❌ GET departments failed:', response.status, await response.text())
      return null
    }
  } catch (error) {
...
async function testGetDepartmentPermissions() {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/admin/department-permissions`, {
      headers: {
        'Authorization': `Bearer ${ADMIN_TOKEN}`
      }
    })
    ...
    if (response.ok) {
      ...
      if (data.permissions && data.permissions.length > 0) {
        ...
        data.permissions.forEach(perm => {
          console.log(`  - ${perm.employee_id} → ${perm.department} (${perm.is_active ? 'Active' : 'Inactive'})`)
        })
      }
      return data.permissions
    } else {
      console.error('❌ GET department permissions failed:', response.status, await response.text())
      return null
    }
  } catch (error) {
...
async function testCreateDepartmentPermission(departments) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('⚠️ POST department permission expected to fail (employee may not exist)')
...
async function testDeleteDepartmentPermission(permissionId) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/admin/department-permissions?id=${permissionId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${ADMIN_TOKEN}`
      }
    ...
    if (response.ok) {
      ...
      console.error('❌ DELETE department permission failed:', response.status, await response.text())
      return false
    }
  } catch (error) {
...
async function testPermissionsSummary() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error('❌ GET permissions summary failed:', response.status, await response.text())
      return null
    }
  } catch (error) {
...
async function runAllTests() {
  ...
  console.log(`✅ Permissions API: ${permissions !== null ? 'Working' : 'Failed'}`)
  console.log(`✅ Create Permission: ${newPermission ? 'Working' : 'Expected to fail (no test employee)'}`)
