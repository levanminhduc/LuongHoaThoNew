...
-- Run this immediately to fix "Lỗi hệ thống khi ký tên"
...
CREATE OR REPLACE FUNCTION auto_sign_salary(
  ...
  p_client_timestamp VARCHAR(50) DEFAULT NULL  -- NEW: Client timestamp parameter
) RETURNS JSONB AS $$
DECLARE
  ...
  result JSONB;
BEGIN
  -- ✅ CHẮC CHẮN 100% - Sử dụng client timestamp hoặc simple math
  IF p_client_timestamp IS NOT NULL AND p_client_timestamp != '' THEN
    -- Parse client timestamp (đã đúng Vietnam time từ client)
    BEGIN
      v_current_time := p_client_timestamp::TIMESTAMP;
      RAISE NOTICE '✅ Using client Vietnam timestamp: %', v_current_time;
    EXCEPTION WHEN OTHERS THEN
      -- Fallback: Simple math +7 hours (CHẮC CHẮN hoạt động)
      v_current_time := CURRENT_TIMESTAMP + INTERVAL '7 hours';
      RAISE NOTICE '⚠️ Client timestamp parse failed, using UTC+7: %', v_current_time;
    END;
  ELSE
    -- Fallback: Simple math +7 hours (CHẮC CHẮN hoạt động trên mọi server)
    v_current_time := CURRENT_TIMESTAMP + INTERVAL '7 hours';
    RAISE NOTICE '📝 Using UTC+7 calculation: %', v_current_time;
  END IF;
...
-- EMERGENCY FIX COMPLETED: auto_sign_salary function updated with client_timestamp parameter
