...
export function useSignatureProgress({
  ...
  const fetchProgress = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Lỗi khi tải tiến độ"
  ...
  const checkForUpdates = (oldData: SignatureProgressData, newData: SignatureProgressData) => {
    if (oldData.employee_progress.signed_count < newData.employee_progress.signed_count) {
      const newSignatures = newData.employee_progress.signed_count - oldData.employee_progress.signed_count
      toast.success("Cập nhật tiến độ", {
        description: `${newSignatures} nhân viên mới đã ký lương`
      })
    }

    if (oldData.employee_progress.is_complete !== newData.employee_progress.is_complete && newData.employee_progress.is_complete) {
      toast.success("🎉 Hoàn thành!", {
        description: "100% nhân viên đã ký lương. Có thể tiến hành ký xác nhận management."
      })
    }

    if (oldData.management_progress.completed_types.length < newData.management_progress.completed_types.length) {
    ...
    if (newData.management_progress.completion_percentage === 100 && 
