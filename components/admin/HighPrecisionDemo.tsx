...
export default function HighPrecisionDemo() {
  ...
  const sampleTestCases = [
    { input: "199.9999999999", field: "he_so_lam_viec", description: "High precision coefficient" },
    { input: "25.1234567890", field: "ngay_cong_trong_gio", description: "High precision days" },
    { input: "150.0000000001", field: "gio_cong_tang_ca", description: "High precision hours" },
    { input: "1,234.567890", field: "he_so_phu_cap_ket_qua", description: "Thousands separator" },
    { input: "99.99%", field: "he_so_luong_co_ban", description: "Percentage format" },
    { input: "1.000,50", field: "ngay_cong_trong_gio", description: "European decimal format" },
    { input: "100000.1", field: "he_so_lam_viec", description: "Value exceeding limit" },
    { input: "-5.5", field: "gio_cong_tang_ca", description: "Negative value (should fail)" }
  ]
  ...
  const runSingleTest = () => {
    ...
    try {
      ...
      const formatted = result.success && result.value !== undefined 
  ...
  const getStatusIcon = (success: boolean, warnings?: string[]) => {
    ...
    if (warnings && warnings.length > 0) return <AlertTriangle className="h-4 w-4 text-yellow-500" />
    return <CheckCircle className="h-4 w-4 text-green-500" />
  }

  const getStatusBadge = (success: boolean, warnings?: string[]) => {
    if (!success) return <Badge variant="destructive">Failed</Badge>
    if (warnings && warnings.length > 0) return <Badge variant="secondary">Warning</Badge>
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Test và demo hệ thống xử lý số có precision cao (lên đến 10 chữ số thập phân)
          ...
          <Tabs defaultValue="single" className="space-y-4">
            ...
            <TabsContent value="batch" className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Batch test sẽ chạy tất cả các test cases mẫu để kiểm tra các tình huống khác nhau
                </AlertDescription>
              </Alert>
            ...
            <TabsContent value="config" className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(FIELD_PRECISION_CONFIG).map(([fieldName, config]) => (
                    <Card key={fieldName} className="p-4">
                      <div className="space-y-2">
                        <div className="font-mono text-sm font-medium">{fieldName}</div>
                        <div className="space-y-1 text-xs">
                          <div>Type: <Badge variant="outline">{config.fieldType}</Badge></div>
                          <div>Max Decimals: {config.maxDecimalPlaces}</div>
                          <div>Max Integer Digits: {config.maxIntegerDigits}</div>
                          <div>Allow Negative: {config.allowNegative ? "Yes" : "No"}</div>
                        </div>
                      </div>
                    </Card>
                  ))}
