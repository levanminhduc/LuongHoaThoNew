...
const AnimatedGradientButton = React.forwardRef<
  HTMLButtonElement,
...
>(({ className, children, variant = "teal", ...props }, ref) => {
  const variantStyles = {
    teal: {
      background:
        "radial-gradient(ellipse 80% 50% at 50% 120%, #037d77, #037d77)",
      hoverShadow: "hover:shadow-[1px_3px_45px_-11px_#03706b]",
    },
    blue: {
      background:
        "radial-gradient(ellipse 80% 50% at 50% 120%, #1e40af, #1e40af)",
      hoverShadow: "hover:shadow-[1px_3px_45px_-11px_#1e3a8a]",
    },
    green: {
      background:
        "radial-gradient(ellipse 80% 50% at 50% 120%, #059669, #059669)",
      hoverShadow: "hover:shadow-[1px_3px_45px_-11px_#047857]",
    },
    default: {
      background:
        "radial-gradient(ellipse 80% 50% at 50% 120%, #6366f1, #6366f1)",
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute inset-[-100%] rotate-gradient">
            <div
              ...
              style={{
                background:
                  "conic-gradient(from calc(270deg - (var(--spread) * 0.5)), transparent 0, hsl(0 0% 100% / 1) var(--spread), transparent var(--spread))",
              }}
            />
          </div>
        </div>
