...
export function MappingConfigOverrideDialog({
  ...
  const [editingConfig, setEditingConfig] = useState<Partial<MappingConfiguration> | null>(null)
  ...
  const handleSaveAsNew = async () => {
    ...
    try {
      if (onSaveAsConfig) {
        onSaveAsConfig(newConfigName.trim(), newConfigDescription.trim() || undefined)
        onOpenChange(false)
      }
    } catch (error) {
    ...
    <div className="space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {config.field_mappings?.length || 0} mappings • 
      ...
      {selectedConfig && (
        <Card>
          ...
          <CardContent>
            {renderConfigPreview(selectedConfig)}
          </CardContent>
        </Card>
      )}
    </div>
          ...
          <CardContent>
            <div className="space-y-2">
              <p className="text-sm text-gray-600">
                This configuration will save {Object.keys(currentMapping).length} field mappings.
              </p>
              <div className="max-h-32 overflow-y-auto">
                <div className="space-y-1 text-xs">
                  {Object.entries(currentMapping).map(([dbField, excelColumn]) => (
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[80vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Browse, create, or edit mapping configurations for import processes.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setEditingConfig(prev => prev ? { ...prev, config_name: e.target.value } : null)}
                    />
                  </div>
                  <div>
      ...
      <Dialog open={showDeleteConfirm !== null} onOpenChange={() => setShowDeleteConfirm(null)}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button 
              variant="destructive" 
              onClick={() => showDeleteConfirm && handleDeleteConfig(showDeleteConfirm)}
            >
              Delete
            </Button>
          </DialogFooter>
