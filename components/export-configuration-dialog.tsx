...
export function ExportConfigurationDialog({
  ...
  const headerPreview = generatePreview(selectedFields, selectedConfigId || undefined)

  // Handle export
  const handleExport = () => {
    const previewHeaders = headerPreview.preview.reduce((acc, item) => {
  ...
  const handleGenerateTemplate = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/generate-import-template?configId=${selectedConfigId}`, {
        method: "GET",
        headers: {
  ...
  const generateFilenamePreview = () => {
    ...
    const timestamp = new Date().toISOString().slice(0, 10)
    const configSuffix = selectedConfig ? `-${selectedConfig.config_name.replace(/\s+/g, '-')}` : ''
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-hidden flex flex-col">
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>
            Configure export options and preview headers before generating Excel file.
          </DialogDescription>
        </DialogHeader>
              ...
              <Card className="flex-shrink-0">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label className="text-sm">Include actual data (not just template)</Label>
                  </div>

                  {/* Salary Month */}
                  {includeData && (
                    <div className="space-y-2">
                      <Label htmlFor="salary-month" className="text-sm">Salary Month</Label>
                      <Input
                        id="salary-month"
                        type="month"
                        value={salaryMonth}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setSelectedConfigId(value && value !== "none" ? parseInt(value) : null)}
                    >
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Select configuration..." />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="none">No configuration (use defaults)</SelectItem>
                    ...
                    {selectedConfig && (
                      <div className="text-xs text-gray-600 space-y-1">
                        {selectedConfig.description && (
                          <p className="truncate">{selectedConfig.description}</p>
                        )}
                        <p>{selectedConfig.field_mappings?.length || 0} field mappings</p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Custom Filename */}
                  <div className="space-y-2">
                    <Label htmlFor="custom-filename" className="text-sm">Custom Filename (Optional)</Label>
                    <Input
                      id="custom-filename"
                ...
                <CardHeader className="pb-3 flex-shrink-0">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="outline" size="sm" onClick={handleSelectNone} className="h-7 px-2 text-xs">
                        None
                      </Button>
                    </div>
                  </div>
                  <CardDescription className="text-xs">
                    Choose fields to include ({selectedFields.length} of {availableFields.length} selected)
                  </CardDescription>
                </CardHeader>
                  ...
                  <ScrollArea className="h-full">
                    <div className="space-y-2 pr-2">
                      {availableFields.map((field) => {
                        ...
                        const fieldPreview = headerPreview.preview.find(p => p.field === field)

                        return (
                          <div key={field} className="flex items-start space-x-2 py-1">
                            <Switch
                              checked={isSelected}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-sm font-medium truncate">{field}</div>
                              {fieldPreview && (
                                <div className="text-xs text-gray-500 truncate">
                                  → {fieldPreview.header}
                                  <Badge
                ...
                <CardContent className="flex-1 min-h-0 space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <Badge variant="default" className="text-xs h-5">{headerPreview.summary.fromConfig}</Badge>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>From Defaults:</span>
                      <Badge variant="secondary" className="text-xs h-5">{headerPreview.summary.fromDefault}</Badge>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span>Fallback:</span>
                      <Badge variant="outline" className="text-xs h-5">{headerPreview.summary.fallback}</Badge>
                    </div>
                  </div>
                    ...
                    <ScrollArea className="h-full">
                      <div className="space-y-2 pr-2">
                        {headerPreview.preview
                          ...
                          .map((item) => (
                          <div key={item.field} className="p-2 border rounded text-sm">
                            <div className="flex items-center justify-between gap-2">
                              <div className="font-medium truncate flex-1">{item.header}</div>
                              <div className="flex items-center gap-1 flex-shrink-0">
                                <Badge
                                  variant={
                                    item.source === 'configuration' ? 'default' :
                                    item.source === 'default' ? 'secondary' : 'outline'
                                  }
                                ...
                                <Badge
                                  ...
                                  className={`text-xs h-4 ${
                                    ...
                                    item.confidence >= 50 ? 'text-yellow-600' : 'text-red-600'
                                  }`}
                                >
                                  {item.confidence}%
                                </Badge>
              ...
              {headerPreview.summary.fallback > 0 && (
                <Alert className="flex-shrink-0">
                  ...
                  <AlertDescription className="text-xs">
                    {headerPreview.summary.fallback} fields are using fallback headers.
                    Consider creating a mapping configuration for better header names.
                  </AlertDescription>
                </Alert>
              )}
            </div>
