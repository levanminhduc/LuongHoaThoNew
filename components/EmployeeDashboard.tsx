...
import { DollarSign, FileCheck, LogOut, Download, Calendar, TrendingUp } from "lucide-react"
          ...
          <div className="flex justify-between items-center py-4">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Xin chào, {user.username} | Mã NV: {user.employee_id} | Department: {user.department}
              </p>
            </div>
              ...
              <Select value={selectedYear.toString()} onValueChange={(value) => setSelectedYear(parseInt(value))}>
                ...
                <SelectContent>
                  {Array.from({ length: 5 }, (_, i) => {
                    ...
                    return (
                      <SelectItem key={`employee-year-${i}-${year}`} value={year.toString()}>
                        {year}
                      </SelectItem>
                    )
                  })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Tổng Lương Năm</CardTitle>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Lương TB/Tháng</CardTitle>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{yearlySummary.signedPercentage}%</div>
                <p className="text-xs text-muted-foreground">
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-semibold">{formatCurrency(yearlySummary.totalGrossSalary)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Tổng lương thực nhận</p>
                        <p className="font-semibold">{formatCurrency(yearlySummary.totalNetSalary)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Tổng thuế TNCN</p>
                        <p className="font-semibold">{formatCurrency(yearlySummary.totalTax)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Tổng BHXH/BHYT</p>
                        <p className="font-semibold">{formatCurrency(yearlySummary.totalInsurance)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <tbody>
                      {payrollData.map((payroll) => (
                        <tr key={payroll.id} className="border-b hover:bg-gray-50">
                          <td className="p-3 font-medium">{payroll.salary_month}</td>
                          ...
                          <td className="p-3 text-center">
                            <Button 
                              variant="outline" 
                              size="sm"
                              onClick={() => handleDownloadPayslip(payroll.id, payroll.salary_month)}
          ...
          <TabsContent value="trends" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Xu hướng lương theo tháng (triệu VND)</CardDescription>
              </CardHeader>
              <CardContent>
