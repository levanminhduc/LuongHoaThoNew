...
export function ColumnMappingManager({
  ...
  const initializeMappings = useCallback(() => {
    const initialMappings: ColumnMapping[] = detectedColumns.map((columnName, index) => {
      // Check if mapping already exists
      const existingMapping = existingMappings.find(m => m.excel_column_name === columnName)
      ...
      const fieldDef = PAYROLL_FIELD_DEFINITIONS.find(f => f.field === suggestedField)
  ...
  const updateMapping = (index: number, field: keyof ColumnMapping, value: any) => {
    ...
    if (field === "database_field") {
      ...
      const fieldDef = PAYROLL_FIELD_DEFINITIONS.find(f => f.field === actualValue)
      if (fieldDef) {
        updatedMappings[index].data_type = fieldDef.data_type
        updatedMappings[index].is_required = fieldDef.is_required
        updatedMappings[index].default_value = fieldDef.default_value?.toString() || ""
      } else if (actualValue === "") {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(`Error saving mappings: ${error instanceof Error ? error.message : "Unknown error"}`)
    } finally {
      setSaving(false)
    }
  }
  ...
  const getMappingStatus = (mapping: ColumnMapping) => {
    ...
    const fieldDef = PAYROLL_FIELD_DEFINITIONS.find(f => f.field === mapping.database_field)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Map Excel columns to database fields for {fileType === "file1" ? "File 1" : "File 2"}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {/* Status Message */}
        {message && (
          <Alert className={message.includes("error") || message.includes("Validation") ? "border-red-200 bg-red-50" : "border-green-200 bg-green-50"}>
            <AlertDescription className={message.includes("error") || message.includes("Validation") ? "text-red-800" : "text-green-800"}>
              {message}
            </AlertDescription>
          </Alert>
        )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                    ...
                    <div className="md:col-span-4">
                      <Select
                        ...
                        onValueChange={(value) => updateMapping(index, "database_field", value)}
                      >
                        <SelectTrigger>
                    ...
                    <div className="md:col-span-3">
                      {fieldDef && (
                        <div className="text-xs text-gray-600">
                          <p><strong>Type:</strong> {fieldDef.data_type}</p>
                          <p><strong>Category:</strong> {fieldDef.category}</p>
