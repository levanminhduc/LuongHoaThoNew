...
export function ColumnMappingAnalysis({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'unmapped': return 'text-red-600 bg-red-50 border-red-200'
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'needs_review': return <AlertTriangle className="h-4 w-4 text-yellow-600" />
      case 'unmapped': return <XCircle className="h-4 w-4 text-red-600" />
      default: return null
    }
  }
  ...
  const getOverallStatus = () => {
    if (mappingSuccessRate >= 90) return { status: 'excellent', color: 'text-green-600', message: 'Excellent mapping quality' }
    if (mappingSuccessRate >= 70) return { status: 'good', color: 'text-blue-600', message: 'Good mapping quality' }
    if (mappingSuccessRate >= 50) return { status: 'fair', color: 'text-yellow-600', message: 'Fair mapping quality' }
    return { status: 'poor', color: 'text-red-600', message: 'Poor mapping quality - needs attention' }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            ...
            {configurationUsed && (
              <span className="ml-2">
                • Configuration: <span className="font-medium">{configurationUsed}</span>
              </span>
            )}
            {aliasesUsed !== undefined && (
              <span className="ml-2">
                • Aliases Available: <span className="font-medium">{aliasesUsed}</span>
              </span>
            )}
          </p>
        </div>
        <Button variant="outline" onClick={onReanalyze} className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{mappedColumns}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-yellow-600">{needsReviewColumns}</div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-red-600">{unmappedColumns}</div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className={`text-sm ${overallStatus.color}`}>{overallStatus.message}</div>
              </div>
            </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="text-center p-2 bg-green-50 rounded">
                <div className="text-lg font-bold text-green-600">{mappingTypeStats.exactMatches}</div>
                <div className="text-xs text-green-600">Exact Matches</div>
              </div>
              <div className="text-center p-2 bg-blue-50 rounded">
                <div className="text-lg font-bold text-blue-600">{mappingTypeStats.aliasMatches}</div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={getConfidenceBadgeVariant(column.confidence)} className="text-xs">
                      {column.confidence}%
                    </Badge>
                  </div>
                </div>
      ...
      {(needsReviewColumns > 0 || unmappedColumns > 0) && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Action Required:</strong> {needsReviewColumns + unmappedColumns} columns need attention before import. 
            Please review and fix mapping issues to ensure data accuracy.
          </AlertDescription>
        </Alert>
      )}
    </div>
