    ...
    <Card className={`${className}`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant={employeeCompletion.is_100_percent_complete ? "default" : "secondary"}>
            {employeeCompletion.is_100_percent_complete ? "Hoàn thành" : "Đang xử lý"}
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className={`text-center p-3 rounded-lg ${getProgressColor(employeeCompletion.completion_percentage)}`}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm text-gray-600">
              {employeeCompletion.signed_employees}/{employeeCompletion.total_employees}
            </span>
          </div>
        ...
        {employeeCompletion.is_100_percent_complete ? (
          <Alert className="border-green-200 bg-green-50">
            ...
            <AlertDescription className="text-green-800">
              🎉 Tuyệt vời! 100% nhân viên đã ký lương. Có thể tiến hành ký xác nhận management.
            </AlertDescription>
          </Alert>
        ) : (
          <Alert className="border-yellow-200 bg-yellow-50">
            ...
            <AlertDescription className="text-yellow-800">
              Còn {employeeCompletion.total_employees - employeeCompletion.signed_employees} nhân viên chưa ký lương. 
              Cần đợi 100% hoàn thành mới có thể ký xác nhận.
            </AlertDescription>
          </Alert>
        )}
        ...
        {showDetails && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-gray-600">{formatTime(lastUpdated)}</span>
            </div>
              ...
              <div className="space-y-2">
                ...
                >
                  {showUnsignedList ? (
                    ...
                    <>
                      ...
                      Xem nhân viên chưa ký ({employeeCompletion.unsigned_employees_sample.length})
                    </>
                  )}
                </Button>
                ...
                {showUnsignedList && (
                  <div className="max-h-40 overflow-y-auto space-y-2 p-3 bg-gray-50 rounded-lg">
                    {employeeCompletion.unsigned_employees_sample.map((employee) => (
                      <div key={employee.employee_id} className="flex justify-between items-center text-sm">
                        <div>
                          <span className="font-medium">{employee.full_name}</span>
                          <span className="text-gray-500 ml-2">({employee.employee_id})</span>
                        </div>
                    ...
                    {employeeCompletion.unsigned_employees_sample.length >= 10 && (
                      <div className="text-center text-sm text-gray-500 pt-2 border-t">
                        ... và {employeeCompletion.total_employees - employeeCompletion.signed_employees - 10} nhân viên khác
                      </div>
                    )}
                  </div>
                )}
