        ...
        {browserInfo && (
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div>
                <strong>Viewport:</strong> {browserInfo.viewport}
              </div>
              <div>
                <strong>Touch Screen:</strong>{" "}
                {browserInfo.touchScreen ? "Yes" : "No"}
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.entries(features).map(([feature, supported]) => (
                <div key={feature} className="flex items-center gap-2">
                  ...
                  <span
                    className={supported ? "text-green-700" : "text-red-700"}
                  >
                    {feature}
                  </span>
                </div>
          ...
          <CardContent className="space-y-2">
            {!features.localStorage && (
              <p className="text-amber-600">
                ⚠️ localStorage is not available. Some features may not work
                properly.
              </p>
            )}
            {!features["Fetch API"] && (
