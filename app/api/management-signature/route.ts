...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!auth || !['admin', 'giam_doc', 'ke_toan', 'nguoi_lap_bieu'].includes(auth.user.role)) {
      return NextResponse.json({ error: "Không có quyền ký xác nhận" }, { status: 403 })
    }
    ...
    if (!signature_type || !['giam_doc', 'ke_toan', 'nguoi_lap_bieu'].includes(signature_type)) {
      return NextResponse.json({ error: "Loại chữ ký không hợp lệ" }, { status: 400 })
    }

    if (auth.user.role !== 'admin' && auth.user.role !== signature_type) {
      return NextResponse.json({ error: "Chức vụ không có quyền ký loại này" }, { status: 403 })
    }
    ...
    if (payrollError) {
      ...
      return NextResponse.json({ error: "Lỗi khi kiểm tra danh sách nhân viên có bảng lương" }, { status: 500 })
    }
    ...
    if (signedError) {
      ...
      return NextResponse.json({ error: "Lỗi khi kiểm tra nhân viên đã ký" }, { status: 500 })
    }
    ...
    if (!is100PercentComplete) {
      return NextResponse.json({
        ...
        details: {
          ...
          completion_percentage: totalCount > 0 ? Math.round((signedCount / totalCount) * 100 * 100) / 100 : 0,
    ...
    if (empError || !employee) {
      return NextResponse.json({ error: "Nhân viên không tồn tại hoặc đã bị khóa" }, { status: 400 })
    }

    if (employee.chuc_vu !== signature_type && auth.user.role !== 'admin') {
      return NextResponse.json({ error: "Chức vụ nhân viên không khớp với loại chữ ký" }, { status: 400 })
    }
    ...
    try {
      ...
      console.log("Management signatures table not available - will create mock signature")
    }
    ...
    try {
      ...
      if (insertError) {
        ...
        return NextResponse.json({ error: "Lỗi khi lưu chữ ký" }, { status: 500 })
      }

      const statusResponse = await fetch(`${request.nextUrl.origin}/api/signature-status/${salary_month}`, {
        headers: {
      ...
      console.log("Management signatures table not available - returning mock response")
