...
function okGeneric() {
  return NextResponse.json({
    success: true,
    message: "Nếu thông tin hợp lệ, mật khẩu đã được cập nhật. Vui lòng thử đăng nhập với mật khẩu mới."
  }, { status: 200 })
}
  ...
  try {
    ...
    if (!isValidCCCD) {
      ...
      const { data: failResult } = await supabase
        .rpc("handle_password_reset_failure", {
    ...
    await logSecurityEvent(
      ...
      { 
        ...
        first_time_change: !employee.password_hash || employee.password_hash === employee.cccd_hash
      }
    )
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: employee, error } = await supabase
      .from("employees")
      .select("password_hash, cccd_hash, last_password_change_at, password_version")
    ...
    const isUsingCCCD = !employee.password_hash || 
