...
export async function GET(request: NextRequest) {
  try {
    ...
    const allUnique = [...new Set(allDepartments?.map(d => d.department) || [])]
    const activeUnique = [...new Set(activeDepartments?.map(d => d.department) || [])]
    ...
    const departmentBreakdown = allUnique.map(dept => {
      const totalInDept = allDepartments?.filter(d => d.department === dept).length || 0
      const activeInDept = activeDepartments?.filter(d => d.department === dept).length || 0
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        departments_with_only_inactive_employees: allUnique.length - activeUnique.length
      },
      departments: {
        all_unique: allUnique,
        active_unique: activeUnique,
        missing_from_active: allUnique.filter(dept => !activeUnique.includes(dept))
      },
      department_breakdown: departmentBreakdown.sort((a, b) => b.total_employees - a.total_employees),
      analysis: {
