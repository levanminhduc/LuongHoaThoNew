...
export async function GET(request: NextRequest) {
  try {
    ...
    if (positionsError) {
      ...
      return NextResponse.json({ error: "Lỗi truy vấn positions" }, { status: 500 })
    }

    // Count positions
    const positionCounts = positions?.reduce((acc: any, emp: any) => {
    ...
    const employeesByPosition = sampleEmployees?.reduce((acc: any, emp: any) => {
      if (!acc[emp.chuc_vu]) acc[emp.chuc_vu] = []
      if (acc[emp.chuc_vu].length < 5) { // Limit to 5 samples per position
    ...
    const permissionsByPosition = permissions?.reduce((acc: any, perm: any) => {
    ...
    const roleAnalysis = Object.keys(positionCounts).map(position => {
      ...
      switch (position) {
        case "admin":
          ...
          break
        case "giam_doc":
          expectedAccess = "Director dashboard + all departments + financial reports"
          canReceivePermissions = true
          break
        case "ke_toan":
          expectedAccess = "Accountant dashboard + payroll management + financial data"
          canReceivePermissions = true
          break
        case "nguoi_lap_bieu":
    ...
    return NextResponse.json({
      ...
      debug: {
        ...
        query: "SELECT DISTINCT chuc_vu, COUNT(*) FROM employees WHERE is_active = true GROUP BY chuc_vu"
      }
    })

  } catch (error) {
    console.error("Debug positions error:", error)
    return NextResponse.json({ error: "Có lỗi xảy ra khi debug positions" }, { status: 500 })
  }
}
