...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      if (!mgmtError && managementSignatures) {
        const completedTypes = managementSignatures.map(
          (sig) => sig.signature_type,
        );
        const remainingTypes = ["giam_doc", "ke_toan", "nguoi_lap_bieu"].filter(
          (type) => !completedTypes.includes(type),
        );
        const mgmtCompletionPercentage =
          Math.round((completedTypes.length / 3) * 100 * 100) / 100;
    ...
    return toErrorResponse(error, {
      fallbackMessage: "Có lỗi xảy ra khi lấy tiến độ ký",
      headers: CACHE_HEADERS.shortPrivate,
    });
  }
}
