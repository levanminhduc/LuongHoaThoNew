...
export async function GET(request: NextRequest) {
  try {
    ...
    if (signatureType && !['giam_doc', 'ke_toan', 'nguoi_lap_bieu'].includes(signatureType)) {
      return NextResponse.json({ error: "Loại chữ ký không hợp lệ" }, { status: 400 })
    }

    if (limit > 100) {
      return NextResponse.json({ error: "Limit tối đa là 100" }, { status: 400 })
    }
    ...
    try {
      ...
      console.log("Management signatures table not available - returning mock data")
      ...
      if (signatureType) {
        signatures = signatures.filter(sig => sig.signature_type === signatureType)
      }
      
      if (months.length > 0) {
        signatures = signatures.filter(sig => months.includes(sig.salary_month))
      }
      
      if (auth.user.role !== 'admin') {
        signatures = signatures.filter(sig => sig.signed_by_id === auth.user.employee_id)
      }
    ...
    signatures.forEach(sig => {
      ...
      if (!monthlyStats[sig.salary_month].signature_types.includes(sig.signature_type)) {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        signature_types_distribution: {
          giam_doc: signatures.filter(s => s.signature_type === 'giam_doc').length,
          ke_toan: signatures.filter(s => s.signature_type === 'ke_toan').length,
          nguoi_lap_bieu: signatures.filter(s => s.signature_type === 'nguoi_lap_bieu').length
        }
      },
