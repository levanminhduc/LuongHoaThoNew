...
const ALL_PAYROLL_FIELDS = [
  ...
  "ngay_cong_chu_nhat", "tien_luong_chu_nhat", "luong_cnkcp_vuot", "tien_tang_ca_vuot",
...
export async function GET(request: NextRequest) {
  try {
    ...
    const selectedFields = fieldsParam ? fieldsParam.split(",") : ALL_PAYROLL_FIELDS
    ...
    if (aliasError) {
      ...
      return NextResponse.json({ error: "Lỗi khi tải column aliases" }, { status: 500 })
    }
    ...
    if (aliases) {
      const fieldAliases = new Map<string, { alias: string; confidence: number }>()
    ...
    if (includeData) {
      ...
      const sampleData = [
        selectedFields.map(field => {
          ...
          if (field.includes("tien") || field.includes("luong")) return "5000000"
          return "0"
        }),
        selectedFields.map(field => {
          ...
          if (field.includes("tien") || field.includes("luong")) return "6000000"
          return "0"
        })
      ]
      dataRows = sampleData
    ...
    const sheetName = includeData ? `Lương ${salaryMonth || 'Tất cả'}` : "Template Lương Aliases"
