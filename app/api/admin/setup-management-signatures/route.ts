...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!auth || !auth.isRole('admin')) {
      return NextResponse.json({ error: "Chỉ admin mới có quyền setup management signatures" }, { status: 403 })
    }
    ...
    try {
      ...
      if (!tableCheck) {
        setupResults.errors.push('Cannot create table via API - please run SQL script manually')
      } else {
      ...
      try {
        ...
        if (!testError) {
          ...
          setupResults.errors.push('Table does not exist - please run migration script')
        }
      } catch (e) {
    ...
    const isSetupSuccessful = setupResults.tableCreated && 
