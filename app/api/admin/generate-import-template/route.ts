...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!configId) {
      return NextResponse.json(
        { error: "Configuration ID là bắt buộc" },
        { status: 400 },
      );
    }
    ...
    if (configError || !config) {
      return NextResponse.json(
        { error: "Configuration không tồn tại" },
        { status: 404 },
      );
    }

    if (
      !config.configuration_field_mappings ||
      config.configuration_field_mappings.length === 0
    ) {
      return NextResponse.json(
        { error: "Configuration không có field mappings" },
        { status: 400 },
      );
    }
    ...
    if (employeeError) {
      console.warn(
        "Could not fetch employee IDs for sample data:",
        employeeError,
      );
    }
    ...
    const sampleData = [
      generateSampleRow(
        config.configuration_field_mappings,
        1,
        realEmployeeIds[0] || "EMP001",
      ),
      generateSampleRow(
        config.configuration_field_mappings,
        2,
        realEmployeeIds[1] || "EMP002",
      ),
      generateSampleRow(
        config.configuration_field_mappings,
        3,
        realEmployeeIds[2] || "EMP003",
      ),
    ];
