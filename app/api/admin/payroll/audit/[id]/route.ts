  ...
  try {
    ...
    try {
      ...
      if (tableError) {
        ...
        if (tableError.message?.includes("relation") || tableError.message?.includes("does not exist")) {
          return NextResponse.json({
            error: "Bảng audit trail chưa được tạo. Vui lòng liên hệ admin để setup.",
            debug: process.env.NODE_ENV === "development" ? tableError : undefined
          }, { status: 500 })
        }
        ...
        if (tableError.code === "42501" || tableError.message?.includes("RLS")) {
          return NextResponse.json({
            error: "Lỗi quyền truy cập audit trail. Vui lòng kiểm tra RLS policies.",
            debug: process.env.NODE_ENV === "development" ? tableError : undefined
          }, { status: 500 })
        }
      ...
      return NextResponse.json({
        ...
        debug: process.env.NODE_ENV === "development" ? accessError : undefined
      }, { status: 500 })
    }
    ...
    if (auditError) {
      ...
      return NextResponse.json({
        ...
        debug: process.env.NODE_ENV === "development" ? auditError : undefined
      }, { status: 500 })
    }

    // Group changes by timestamp and reason
    const groupedChanges = auditData?.reduce((groups: any[], log: any) => {
