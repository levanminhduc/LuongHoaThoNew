  ...
  try {
    ...
    if (payrollCountError || employeeCountError) {
      ...
      if (payrollExistsError || employeeExistsError) {
        ...
        return NextResponse.json({
          error: "Lỗi truy cập database. Vui lòng kiểm tra cấu hình RLS policies.",
      ...
      if (!payrollExists || payrollExists.length === 0) {
        return NextResponse.json({
          ...
          results: [],
          total: 0,
          message: "Chưa có dữ liệu lương trong hệ thống. Vui lòng import dữ liệu trước."
        })
      }
      ...
      if (!employeeExists || employeeExists.length === 0) {
        return NextResponse.json({
          ...
          results: [],
          total: 0,
          message: "Chưa có dữ liệu nhân viên trong hệ thống. Vui lòng import dữ liệu trước."
        })
      }
    }
