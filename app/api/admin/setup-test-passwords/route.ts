...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!auth || !auth.isRole('admin')) {
      return NextResponse.json({ error: "Chỉ admin mới có quyền setup passwords" }, { status: 403 })
    }
    ...
    return NextResponse.json({ error: "Có lỗi xảy ra khi setup passwords" }, { status: 500 })
  }
}
