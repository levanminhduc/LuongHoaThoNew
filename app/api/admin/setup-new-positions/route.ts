...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!auth || !auth.isRole('admin')) {
      return NextResponse.json({ error: "Chỉ admin mới có quyền setup positions" }, { status: 403 })
    }
    ...
    for (const emp of sampleEmployees) {
      ...
      if (error) {
        ...
        insertResults.push({ employee_id: emp.employee_id, status: 'error', error: error.message })
      } else {
    ...
    return NextResponse.json({ error: "Có lỗi xảy ra khi setup positions" }, { status: 500 })
  }
}
