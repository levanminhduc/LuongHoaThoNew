      ...
      if (seenKeys.has(key)) {
        ...
        if (!duplicateMap.has(key)) {
          ...
          duplicateMap.set(key, [
            { ...records[firstIndex], _batch_index: firstIndex },
          ]);
        }
...
function categorizeError(
  ...
  if (
    ...
    severity = "high";
    suggestion =
      "Ensure all required fields (employee_id, salary_month) have valid values";
    expectedFormat = "Non-empty text or number";
  } else if (
    lowerMessage.includes("invalid date") ||
    lowerMessage.includes("date format")
  ) {
    errorType = "format";
    severity = "medium";
    ...
    lowerMessage.includes("number format")
  ) {
    errorType = "format";
    severity = "medium";
    suggestion =
      "Use numeric values only. Decimal separator: dot (.) or comma (,)";
...
function parseNumberField(value: string, columnName: string): number {
  try {
    ...
    if (isNaN(parsed)) {
      throw new Error(
        `Cannot convert '${value}' to number. Cleaned value: '${cleanedValue}'`,
      );
    }
    ...
    if (
      columnLower.includes("tong_he_so_quy_doi") ||
      columnLower.includes("tổng hệ số quy đổi")
    ) {
      ...
      if (parsed > 9999999999999.99) {
        throw new Error(
          `Value too large: ${parsed}. Maximum: 9,999,999,999,999.99`,
        );
      }
      ...
      if (parsed > 100000000) {
        // 100 million
        console.warn(
          `Very high tong_he_so_quy_doi value detected: ${parsed} for column ${columnName}`,
        );
      }
      ...
      if (parsed > 999.99) {
        throw new Error(
          `Coefficient value too large: ${parsed}. Maximum: 999.99`,
        );
      }
      // Business logic warning for high coefficients
      if (parsed > 100) {
        console.warn(
          `High coefficient value detected: ${parsed} for column ${columnName}`,
        );
      }
    } else if (
      columnLower.includes("luong") ||
      columnLower.includes("salary") ||
      columnLower.includes("tien")
    ) {
      ...
      if (parsed > 9999999999999.99) {
        throw new Error(
          `Salary value too large: ${parsed}. Maximum: 9,999,999,999,999.99`,
        );
      }
      ...
      if (parsed > 744) {
        // 744 = max hours in a month (31 days × 24 hours)
        console.warn(
          `High hours value detected: ${parsed} for column ${columnName} (monthly max typically 744)`,
        );
      }
      ...
      if (parsed > 31) {
        console.warn(
          `High days value detected: ${parsed} for column ${columnName} (monthly max typically 31)`,
        );
      }
    } else {
      ...
      if (parsed < -1000000000 || parsed > 1000000000) {
        throw new Error(
          `Value out of range: ${parsed}. Expected range: -1,000,000,000 to 1,000,000,000`,
        );
      }
    }
    ...
    if (
      ...
      columnLower.includes("gio") ||
      columnLower.includes("ngay") ||
      columnLower.includes("hour") ||
    ...
    console.log(
      `Number parsing successful: '${value}' -> ${rounded} (column: ${columnName})`,
    );

    return rounded;
  } catch (error) {
    throw new Error(
      `Invalid number format in column '${columnName}': '${value}'. ${error instanceof Error ? error.message : "Unknown parsing error"}`,
    );
  }
}
  ...
  try {
    if (typeof value === "number") {
      ...
      if (value < 1 || value > 50000) {
        // Reasonable date range (1900-2036)
        throw new Error(
          `Invalid Excel date serial number: ${value}. Expected range: 1-50000`,
        );
      }
      ...
      if (isNaN(excelDate.getTime())) {
        throw new Error(
          `Invalid Excel date conversion for serial number: ${value}`,
        );
      }
    ...
    if (!stringValue || stringValue.length < 4) {
      throw new Error(
        `Date value too short: '${stringValue}'. Minimum 4 characters required`,
      );
    }
        ...
        try {
          switch (format.type) {
            ...
            case "D-M-YYYY":
              ...
              parsedDate = new Date(
                `${year3}-${month3.padStart(2, "0")}-${day3.padStart(2, "0")}`,
              );
              break;
    ...
    if (!parsedDate) {
      ...
      const yearMonthMatch =
        stringValue.match(/(\d{4}).*?(\d{1,2})/) ||
        stringValue.match(/(\d{1,2}).*?(\d{4})/);
    ...
    if (!parsedDate || isNaN(parsedDate.getTime())) {
      ...
      throw new Error(
        `Invalid date: '${stringValue}'. Supported formats: ${supportedFormats.join(", ")}`,
      );
    }
    ...
    console.log(
      `Date parsing successful: '${stringValue}' (${matchedFormat}) -> '${result}'`,
    );

    return result;
  } catch (error) {
    throw new Error(
      `Invalid date format in column '${columnName}': '${value}'. ${error instanceof Error ? error.message : "Unknown date parsing error"}`,
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const duplicateStrategy =
      (formData.get("duplicateStrategy") as string) || "skip";
      ...
      try {
          ...
          if (mapping && mapping.database_field) {
            ...
            if (cleanedValue !== null && cleanedValue !== "") {
              try {
                ...
                const autoFixResult = attemptAutoFix(
                  cellValue,
                ...
                if (autoFixResult && autoFixResult.success) {
                  ...
                  throw new Error(
                    `${conversionError instanceof Error ? conversionError.message : `Invalid ${mapping.data_type} format in column '${header}': ${cellValue}`}`,
                  );
                }
              }
        ...
        if (mappedData.employee_id && mappedData.salary_month) {
          ...
          if (existingRecord) {
            ...
            switch (duplicateAction) {
              case "skip":
                throw new Error(
                  `Duplicate employee_id found: ${mappedData.employee_id} for month ${mappedData.salary_month}. Skipping record.`,
                );
              ...
              case "overwrite":
                ...
                if (updateError) {
                  throw new Error(
                    `Failed to update duplicate record: ${updateError.message}`,
                  );
                }

                console.log(
                  `Updated duplicate record for employee ${mappedData.employee_id}, month ${mappedData.salary_month}`,
                );
                successCount++;
              ...
              case "merge":
                ...
                const mergedData = resolveDuplicateData(
                  existingRecord,
                  mappedData,
                  "merge",
                );
                ...
                if (mergeError) {
                  throw new Error(
                    `Failed to merge duplicate record: ${mergeError.message}`,
                  );
                }

                console.log(
                  `Merged duplicate record for employee ${mappedData.employee_id}, month ${mappedData.salary_month}`,
                );
                successCount++;
                continue; // Skip the insert below
              ...
              case "create_new":
                ...
                const newData = resolveDuplicateData(
                  existingRecord,
                  mappedData,
                  "create_new",
                );
              ...
              default:
                throw new Error(
                  `Unknown duplicate resolution strategy: ${duplicateAction}`,
                );
            }
          }
        }
        ...
        if (insertError) {
          ...
          if (insertError.code === "23505") {
            ...
            throw new Error(
              `Duplicate record: employee_id ${mappedData.employee_id} already exists for ${mappedData.salary_month}`,
            );
          }
