...
export async function POST(request: NextRequest) {
  try {
    ...
    const { errors, originalData, fileName = "import_errors", format = "excel" } = body

    if (!errors || errors.length === 0) {
      return NextResponse.json({ error: "No errors to export" }, { status: 400 })
    }
    ...
    const summaryData = [
      ...
      { "Metric": "Validation Errors", "Count": errors.filter(e => e.errorType === "validation").length },
      { "Metric": "Format Errors", "Count": errors.filter(e => e.errorType === "format").length },
      { "Metric": "Duplicate Errors", "Count": errors.filter(e => e.errorType === "duplicate").length },
      { "Metric": "Database Errors", "Count": errors.filter(e => e.errorType === "database").length },
      { "Metric": "System Errors", "Count": errors.filter(e => e.errorType === "system").length },
      { "Metric": "High Severity", "Count": errors.filter(e => e.severity === "high" || e.severity === "critical").length },
      { "Metric": "Medium Severity", "Count": errors.filter(e => e.severity === "medium").length },
      { "Metric": "Low Severity", "Count": errors.filter(e => e.severity === "low").length }
    ]
    ...
    if (format === "excel") {
      ...
      // Instructions sheet
      const instructions = [
        { "Step": 1, "Instruction": "Review the 'Error Details' sheet to understand all errors" },
        { "Step": 2, "Instruction": "Check the 'Summary' sheet for error statistics" },
        { "Step": 3, "Instruction": "Use the 'Fix Template' sheet to plan corrections" },
        { "Step": 4, "Instruction": "Fill in 'Corrected Value' column with proper values" },
