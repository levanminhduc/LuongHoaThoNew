...
export function CCCDUpdateForm({ onSubmit, isLoading, employeeName }: CCCDUpdateFormProps) {
  ...
  const isFormValid = newCCCD.length === 12 && confirmCCCD.length === 12 && newCCCD === confirmCCCD
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Alert className="border-blue-500 bg-blue-50">
        ...
        <AlertDescription className="text-blue-800">
          <strong>Lưu ý bảo mật:</strong> Số CCCD sẽ được mã hóa bằng bcrypt trước khi lưu vào database.
          Việc cập nhật này sẽ thay thế hoàn toàn số CCCD cũ của nhân viên <strong>{employeeName}</strong>.
        </AlertDescription>
      </Alert>
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            ...
            {confirmCCCD.length > 0 && newCCCD.length > 0 && (
              <span className={`ml-2 ${newCCCD === confirmCCCD ? "text-green-600" : "text-red-600"}`}>
      ...
      <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
        <strong>Chú ý:</strong> Sau khi cập nhật thành công, nhân viên sẽ cần sử dụng số CCCD mới 
        để đăng nhập và tra cứu thông tin lương. Vui lòng thông báo cho nhân viên về thay đổi này.
      </div>
    </form>
  )
}
