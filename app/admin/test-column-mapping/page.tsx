...
export default function TestColumnMappingPage() {
  ...
  const runBasicTest = async () => {
    setLoading(true)
    try {
      const result = await autoMapColumnsWithAliases(TEST_EXCEL_COLUMNS, aliases)
  ...
  const runProblematicTest = async () => {
    setLoading(true)
    try {
      const result = await autoMapColumnsWithAliases(TEST_PROBLEMATIC_COLUMNS, aliases)
  ...
  const runValidationTest = async () => {
    ...
    try {
      ...
      const validation = validator.validateMapping(TEST_EXCEL_COLUMNS, testResults.basic.mapping)
  ...
  const runAllTests = async () => {
    ...
    try {
      // Run basic test
      const basicResult = await autoMapColumnsWithAliases(TEST_EXCEL_COLUMNS, aliases)
      
      // Run problematic test
      const problematicResult = await autoMapColumnsWithAliases(TEST_PROBLEMATIC_COLUMNS, aliases)
      
      // Run validation test
      const validator = new EnhancedImportValidator(aliases)
      const validationResult = validator.validateMapping(TEST_EXCEL_COLUMNS, basicResult.mapping)
  ...
  const getMappingTypeColor = (type: string) => {
    switch (type) {
      ...
      case "alias": return "bg-purple-100 text-purple-800"
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Test Column Mapping System</h1>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            <div className="mt-4 text-sm text-gray-600">
              <p><strong>Aliases loaded:</strong> {aliases.length}</p>
              <p><strong>Database fields:</strong> {PAYROLL_FIELD_CONFIG.length}</p>
            </div>
          </CardContent>
        </Card>
                ...
                {testResults.basic ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div className="text-center p-4 bg-yellow-50 rounded-lg">
                        <div className="text-2xl font-bold text-yellow-700">
                          {testResults.basic.confidence_summary.medium_confidence}
                        </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {Object.entries(testResults.basic.mapping).map(([column, config]) => (
                        <div key={column} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                          <div>
                            <span className="font-medium">{column}</span>
                            <span className="text-gray-500 mx-2">→</span>
                            <span>{PAYROLL_FIELD_CONFIG.find(f => f.field === config.database_field)?.label}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Badge className={getMappingTypeColor(config.mapping_type)}>
                              {config.mapping_type}
                            </Badge>
                            <Badge className={getConfidenceColor(config.confidence_score)}>
                    ...
                    {testResults.basic.unmapped_columns.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-orange-700">Unmapped Columns:</h4>
                ...
                {testResults.problematic ? (
                  <div className="space-y-4">
                    ...
                    <div className="text-sm">
                      <strong>Mapped:</strong> {Object.keys(testResults.problematic.mapping).length} / {TEST_PROBLEMATIC_COLUMNS.length}
                    </div>
                    <div className="text-sm">
                      <strong>Success:</strong> {testResults.problematic.success ? "✅ Yes" : "❌ No"}
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-8 text-gray-500">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div className="text-center p-4 bg-blue-50 rounded-lg">
                        ...
                        <div className="text-sm text-blue-600">Mapped Columns</div>
                      </div>
                      <div className="text-center p-4 bg-red-50 rounded-lg">
                        <div className="text-2xl font-bold text-red-700">
                    ...
                    <div className="text-lg font-semibold">
                      Validation Status: {testResults.validation.isValid ? 
